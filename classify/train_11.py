...
for file in file_list5:
    ...
    if file_index < 24:
        features_train, labels_train = create_dataset(x, y, seq_len=1, len_step=1, features=features_train,
                                                      labels=labels_train)
    elif file_index < 27:
        features_val, labels_val = create_dataset(x, y, seq_len=1, len_step=1, features=features_val,
                                                  labels=labels_val)
    else:
        features_test, labels_test = create_dataset(x, y, seq_len=1, len_step=1, features=features_test,
                                                    labels=labels_test)
...
def train(net, device, train_loader, criterion, optimizer, num_epochs, val_loader, save_path, writer):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (train_data, train_label) in enumerate(train_bar):
            ...
            train_data = train_data.reshape(-1, 16, 1)
        ...
        with torch.no_grad():
            for step, (val_data, val_label) in enumerate(val_bar):
                val_data = val_data.reshape(-1, 16, 1)
...
def predict(test_num, save_path, test_loader, device, net, times):
    ...
    with torch.no_grad():
        ...
        for step, (test_data, test_label) in enumerate(test_bar):
            # print(test_data)
            # print(test_label)
            test_data = test_data.reshape(-1, 16, 1)
...
if __name__ == "__main__":
    ...
    save_path = './train_11/save_path/time_%d' % times + '.pth'
    writer = SummaryWriter('./train_11/writer/time_%d' % times)
    ...
    predict_result.to_csv('./train_11/result.csv', mode='a', index=None, header=None)
    ...
    compare_result.to_csv('./train_11/compare/time_%d' % times + '.csv', mode='w', index=0, header=None)
