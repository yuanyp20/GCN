...
def train(net, device, train_loader, criterion, optimizer, num_epochs, val_loader, save_path, writer):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (train_data, train_label) in enumerate(train_bar):
            ...
            train_data = train_data.reshape(-1, 16, 50)
        ...
        with torch.no_grad():
            for step, (val_data, val_label) in enumerate(val_bar):
                val_data = val_data.reshape(-1, 16, 50)
...
def predict(test_num, save_path, test_loader, device, net, times):
    ...
    with torch.no_grad():
        ...
        for step, (test_data, test_label) in enumerate(test_bar):
            # print(test_data)
            # print(test_label)
            test_data = test_data.reshape(-1, 16, 50)
...
if __name__ == "__main__":
    ...
    save_path = './train_10/save_path/time_%d' % times + '.pth'
    writer = SummaryWriter('./train_10/writer/time_%d' % times)
    ...
    predict_result.to_csv('./train_10/result.csv', mode='a', index=None, header=None)
    ...
    compare_result.to_csv('./train_10/compare/time_%d' % times + '.csv', mode='w', index=0, header=None)
