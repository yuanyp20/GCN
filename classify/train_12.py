# data: h5_50
# model: gcn_lstm
# window_len: 20，30，35，40，42，44，46，47，48，49
# loss: ce loss
# loss setting: weight
import random
...
def create_dataset(x, y, seq_len, len_step, features, labels):
    for i in range(0, len(x) - seq_len, len_step):
        data = x[i + 20]  # 序列数据
        data = np.append(data, x[i + 30])
        data = np.append(data, x[i + 35])
        data = np.append(data, x[i + 40])
        data = np.append(data, x[i + 42])
        data = np.append(data, x[i + 44])
        data = np.append(data, x[i + 46])
        data = np.append(data, x[i + 47])
        data = np.append(data, x[i + 48])
        data = np.append(data, x[i + 49])
...
for file in file_list5:
    ...
    if file_index < 24:
        features_train, labels_train = create_dataset(x, y, seq_len=50, len_step=1, features=features_train,
                                                      labels=labels_train)
    elif file_index < 27:
        features_val, labels_val = create_dataset(x, y, seq_len=50, len_step=1, features=features_val,
                                                  labels=labels_val)
    else:
        features_test, labels_test = create_dataset(x, y, seq_len=50, len_step=1, features=features_test,
                                                    labels=labels_test)
...
def train(net, device, train_loader, criterion, optimizer, num_epochs, val_loader, save_path, writer):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (train_data, train_label) in enumerate(train_bar):
            ...
            train_data = train_data.reshape(-1, 16, 10)

            data = Data(x=train_data, edge_index=edge_index.t().contiguous(),
        ...
        with torch.no_grad():
            for step, (val_data, val_label) in enumerate(val_bar):
                val_data = val_data.reshape(-1, 16, 10)

                data = Data(x=val_data, edge_index=edge_index.t().contiguous(),
                            y=torch.tensor(val_label, dtype=torch.long))
                val_data = data
                val_label = data.y
                val_data = val_data.to(device)
                val_label = val_label.to(device)
                val_preds = net(val_data, hs)
                hs = tuple([h.data for h in hs])
...
def predict(test_num, save_path, test_loader, device, net, times):
    ...
    with torch.no_grad():
        ...
        for step, (test_data, test_label) in enumerate(test_bar):
            # print(test_data)
            # print(test_label)
            test_data = test_data.reshape(-1, 16, 10)
            data = Data(x=test_data, edge_index=edge_index.t().contiguous(),
                        y=torch.tensor(test_label, dtype=torch.long))
            test_data = data
            test_label = data.y
            test_data = test_data.to(device)
            test_label = test_label.to(device)
            test_preds = net(test_data, hs)
            hs = tuple([h.data for h in hs])
...
if __name__ == "__main__":
    ...
    save_path = './train_12/save_path/time_%d' % times + '.pth'
    writer = SummaryWriter('./train_12/writer/time_%d' % times)
    ...
    predict_result.to_csv('./train_12/result.csv', mode='a', index=None, header=None)
    ...
    compare_result.to_csv('./train_12/compare/time_%d' % times + '.csv', mode='w', index=0, header=None)
