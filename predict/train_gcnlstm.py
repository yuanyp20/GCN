...
def train(net, device, train_loader, criterion, optimizer, num_epochs, val_loader, save_path, writer):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (train_data, train_label) in enumerate(train_bar):
            ...
            # 得到的loss是batch内损失的平均
            loss = criterion(y_hat.float(), train_label.float())
        ...
        with torch.no_grad():
            for step, (val_data, val_label) in enumerate(val_bar):
                ...
                data = Data(x=val_data, edge_index=edge_index.t().contiguous(),
                            y=torch.tensor(val_label, dtype=torch.float))
                val_data = data
                val_label = data.y
                ...
                hs = tuple([h.data for h in hs])
                loss = criterion(val_preds.float(), val_label.float())  # 计算损失
                val_loss += loss.item()
...
def predict(test_num, save_path, test_loader, device, net, times):
    ...
    with torch.no_grad():
        ...
        for step, (test_data, test_label) in enumerate(test_bar):
            ...
            test_loss = criterion(test_label.float(), test_preds.float())
            # print('score',score)
...
if __name__ == "__main__":
    ...
    save_path = './train_1/save_path/time_%d' % times + '.pth'
    writer = SummaryWriter('./train_1/writer/time_%d' % times)
    ...
    predict_result.to_csv('./train_1/result.csv', mode='a', index=None, header=None)
